...
async def sample_trip(db_session):
    """Create a sample trip in the database, yield it, then clean it up."""
    trip = Trip(
        event_name="Sample Test Trip",
        event_slug="sample-test-trip-fixture",
        start_time=datetime.fromisoformat("2025-06-01T10:00:00").date(),
        end_time=datetime.fromisoformat("2025-06-01T15:00:00").date(),
        organizers=[
            {"original": "Test Organizer", "normalized": "test organizer", "has_diacritics": False, "role": "guide"}
        ],
    ...
    # Cleanup: delete the trip after the test
...
class TestBeesActiveEndpoint:
    ...
    def test_bees_active_basic(self, test_client, snapshot):
        """Snapshot test: basic call to bees/active with default parameters (full payload)."""
        ...
        data = response.json()
        snapshot.assert_match(json.dumps(data, indent=4), "bees_active_basic.json")

    def test_bees_active_with_date_range(self, test_client, snapshot):
        """Snapshot test: bees/active with explicit date range."""
        response = test_client.get(
            "/bees/active",
            params={
                "lat": 37.7749,
    ...
    def test_bees_active_sort(self, test_client, sort_param, reverse):
        """Test bees/active with different sort parameters."""
        response = test_client.get(
            "/bees/active",
            params={
                "lat": 37.7749,
...
class TestTripsEndpoints:
    ...
    def test_create_trip_basic(self, test_client):
        ...
        assert data["organizers"][0]["original"] == organizer_name
    ...
    def test_create_trip_with_location(self, test_client):
        ...
        response = test_client.post(
            ...
            json={
                "event_name": event_name,
                "organizers": [{"display_name": fake.name()}],
    ...
    def test_get_trip_basic(self, test_client, sample_trip):
        """Test getting a trip by ID."""
        get_response = test_client.get(f"/trips/{sample_trip.id}")
        assert get_response.status_code == 200
        data = get_response.json()
        assert data["id"] == sample_trip.id
        assert data["event_name"] == sample_trip.event_name
    ...
    def test_list_trips_basic(self, test_client, sample_trip):
        """Test listing trips."""
        ...
        # Verify our sample trip is in the list
        trip_ids = [trip["id"] for trip in data["data"]]
        assert sample_trip.id in trip_ids
    ...
    def test_create_trip_organizer_with_role(self, test_client):
        ...
        response = test_client.post(
            ...
            json={
                "event_name": event_name,
                "organizers": [
                    {"display_name": fake.name(), "role": "lead"},
                    {"display_name": fake.name(), "role": "assistant"},
                ],
